/* This version modified by Daniel Wagenaar to output doubles to file
   Changes (C) 2013 Daniel A Wagenaar
...
void print_dpss(FILE *out, dpss *mydpss);                               //prints the results of dpss to stdout
void print_dpss(FILE *out, const double *h, uint_t n, const double *l, int nseq);  //prints the results of dpss to stdout
...
"\n\
Usage: mkdpss outputfile n nW [[seql] sequ] [interp_method [interp_base]] [trace]\n\
\n\
...
multitaper spectral analysis.\n\
    \n\
Examples:\n\
dpss out.dat 12 3 2        returns the first two DPSSs of length 12 and time-bandwidth\n\
                   product nW=3, along with their concentrations.\n\
dpss out.dat 12 3 3 5      returns the third through fifth DPSSs and concentrations.\n\
dpss out.dat 8388608 2.5 spline 32768   computes the first five DPSSs of length 2^15,\n\
...
\n\
Required Parameters:\n\
  outputfile - Filename where the results will be saved\n\
...
void print_dpss(FILE *out, dpss *mydpss){
    
  print_dpss(out, mydpss->ph(), mydpss->size(0), mydpss->pl(), mydpss->size(1));
    
}

void print_dpss(FILE *out, const double *h, uint_t n, const double *, int nseq){
  fwrite(h, sizeof(double), n*nseq, out);
}
