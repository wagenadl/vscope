...
for j=1:n_rois
  ...
  line([t_max+T_bar/2 t_max+T_bar/2],...
       ...
       'Color',[0 0 0],...
       'LineWidth',3);
  % star the trace if requested
  if roi_starred(j)
    xl=xlim; yl=ylim;
    x_star=0.02*xl(1)+0.98*xl(2);
    y_star=0.05*yl(1)+0.95*yl(2);
    line(x_star,y_star,...
         'LineStyle','none','Marker','*','MarkerSize',3,'Color',[0 0 0]);
  end
  % draw the label
...
%end
scale_string=sprintf('%s%.0f ms',scale_string,t_bar_size);
text('Units','normalized',...
